...
class GroupExpandableRecyclerViewAdapter(private val setDrawable:(CheckedTextView, Boolean)-> Unit, private val actionModeCallback: ManagementFragmentActionModeCallBack, private val selectedPersons: Set<String>, private val selectedGroups: Set<String>, private val groupMap:Map<String, Group>, personGroups: List<ExpandableGroup<*>>, private val personClickListener: (Checkable, Person) -> Unit, private val groupClickListener: (Checkable, Group) -> Unit) : ExpandableRecyclerViewAdapter<GroupViewHolder, PersonViewHolder>(personGroups) {
    ...
    override fun onBindGroupViewHolder(holder: GroupViewHolder, flatPosition: Int, group: ExpandableGroup<*>) = holder.bind(groupMap.getValue(group.title), actionModeCallback.multiSelect)
}
